...
class ProductRepository {
    async addProductToStore({ title, description, price, img, code, stock, category, thumbnails }) {
        try {
    ...
    async getProductsFromStore(limit = 10, page = 1, sort, query) {
        try {
    ...
    async updateProductFromStore(id, updatedProduct) {
        try {
    ...
    async deleteProductFromStore(id) {
        try {
            const removed = await ProductsModel.findByIdAndDelete(id);
